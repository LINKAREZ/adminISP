        ...
        <div class="col-md-8 offset-md-2">
            <form method="POST" action="{{ route('red.routers.update', $router) }}" id="form-router-edit">
                ...
                <x-card title="Editar Router" icon="fa-network-wired" variant="primary">
                        ...
                        @if(isset($licencias) && $licencias->isNotEmpty())
                        <!-- Licencia -->
                                ...
                                <div class="row">
                                    <div class="col-12 col-md-4">
                                        <div class="form-group mb-2 mb-md-0">
                                            ...
                                            <select id="licencia_id" name="licencia_id" class="form-control @error('licencia_id') is-invalid @enderror">
                                                <option value="">Sin licencia</option>
                                                @foreach($licencias as $p)
                                                    <option value="{{ $p->id }}" {{ old('licencia_id', $router->licencia_id) == $p->id ? 'selected' : '' }}>{{ $p->name }} ({{ $p->max_clientes ?? '—' }} clientes)</option>
                                                @endforeach
                                            </select>
                                            @error('licencia_id')<span class="invalid-feedback">{{ $message }}</span>@enderror
                                        </div>
                                    </div>
                                    <div class="col-12 col-md-4">
                                        <div class="form-group mb-2 mb-md-0">
